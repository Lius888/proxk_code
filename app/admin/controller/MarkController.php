 ...
 * @Description		生产工艺
 * @Author			LiuS
 * @Date			2020-11-02 15:14:52
 * @LastEditTime	2020-11-02 15:14:52
 */
...
class MarkController extends AdminBaseController
{
    ...
    public function confirm(){
        ...
        if(empty($id))              $this->error("参数错误！");
        if (Db::name('porder')->where('oid', $id)->data(['status' => 1,'edit_time'=> time()])->update() !== false) {
            $this->success("确认成功！");
        } else {
            $this->error("确认失败！");
        }
